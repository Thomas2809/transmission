...
TwoWire secondWire(1); // Utilisation de TwoWire pour le deuxième bus I2C
...
bool DFRobot_ADS1115::checkADS1115()
{
    uint8_t error;
    ads_i2cAddress;
    secondWire.beginTransmission(ads_i2cAddress); // Utilisation du second bus I2C (Wire1)
    error = secondWire.endTransmission(); // Utilisation du second bus I2C (Wire1)
    if(error == 0){
        return true;
    }else{
        return false;
